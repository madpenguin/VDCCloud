...
int doSetup(char* host,char* name,uint64_t *size64)
{
    uint32_t flags;
...
void doServe(struct thread_info* tinfo)
{
	syslog(LOG_INFO,"THREAD STARTED");
    if(!tinfo->host) return;
...
//		return;
  //  }    
   ...
   // }
    ...
    //do {
        sock = doSetup(tinfo->host,tinfo->name,nbd,&size);
        if(sock<0) {
            syslog(LOG_ALERT,"Unable to connect to host [%s] - retry in 10s (%d:%d)",tinfo->host,sock,errno);
			return;
			//sleep(30);
			//continue;
        }
    ...
    //} while( running );
	syslog(LOG_INFO,"THREAD STOPPED");
}
*/
    ...
    do {
		...
		switch( opt )
		{
	        case NBD_OPT_EXPORT_NAME:                
		        ...
		        int sock1 = doSetup(host1,name,&size);
				if(sock1==-1) {
					syslog(LOG_ALERT,"Unable to connect to host [%s] - retry in 10s (%d:%d)",host1,sock1,errno);
					return False;
				}
				syslog(LOG_INFO,"%s :: Negotiated size=%lld",host1,(unsigned long long) size);
				int sock2 = doSetup(host1,name,&size);
				if(sock2==-1) {
					syslog(LOG_ALERT,"Unable to connect to host [%s] - retry in 10s (%d:%d)",host2,sock2,errno);
					return False;
				}
...
/*			pthread_t thread1;
			...
			if( pthread_create(&tinfo1.thread_id,&attr,doServe,&tinfo1) != 0 ) {
				syslog(LOG_ALERT,"Error creating thread, err=%d",errno);
				return False; 
			}
			...
			if( pthread_create(&tinfo2.thread_id,&attr,doServe,&tinfo2) != 0 ) {
				syslog(LOG_ALERT,"Error creating thread, err=%d",errno);
				return False; 
			}	
            sleep(1);
		...
		do {
			...
			case NBD_WRITE:
				...
				//	block++;
					...
					//			
					//while( left > 0 ) {
						//if(!cacheWrite(block,bufp)) syslog(LOG_ERR,"Write Error on %lld",(unsigned long long)block);						
						//block++;
						//bufp += 1024;
						//left -= 1024;
					//}
